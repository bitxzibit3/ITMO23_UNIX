    ...
    for (size_t i = 0; i < size; i++) {
        ...
        switch (ct) {
            case 1: {
                ...
                threads.emplace_back([this](int begin, int end, std::string to_exec, int step) {
                    this->make_loop(begin, end, to_exec, step);
                    auto id = std::this_thread::get_id();
                    log_operation(logfile, to_exec, id);
//                    std::cout << "[" << id << "]";
                }, args[0], args[1], command_to_execute, args[2]);
                break;
            }
            default: {
                threads.emplace_back([this](std::string command) {
                    this->make_command(command);
                    auto id = std::this_thread::get_id();
                    std::cout << "[" << id << "]";
                }, command);
                break;
...
void log_operation(FileHandler logfile, std::string operation, std::thread::id thread_id) {
    std::stringstream ss;
    ss << thread_id;
